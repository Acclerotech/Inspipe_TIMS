...
const NAV_ITEMS: NavItemDef[] = [
  ...
  { to: '/upload', icon: Database, label: 'Data Management' },
...
export default function Sidebar() {
  ...
  return (
    <aside className={`${collapsed ? 'w-16' : 'w-56'} shrink-0 bg-[#0f1729] flex flex-col h-screen sticky top-0 z-20 transition-all duration-200`}>
      ...
      <div className="px-4 py-4 border-b border-slate-700/40 flex items-center gap-3">
        ...
        {!collapsed && (
          <div>
            <p className="text-white font-bold text-[12px] leading-tight tracking-wider">INLINE TANK</p>
            <p className="text-white font-bold text-[12px] leading-tight tracking-wider">INSPECTION</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {NAV_ITEMS.map(({ to, icon: Icon, label, badge, children }) => {
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `flex items-center gap-2 px-3 py-1.5 rounded text-xs transition-colors ${
                            isActive ? 'text-white bg-blue-600/80 font-medium' : 'text-slate-400 hover:text-white hover:bg-slate-700/40'
                          }`
                        }
                      >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded transition-colors relative ${
                  isActive ? 'text-white bg-blue-600 font-medium' : 'text-slate-300 hover:text-white hover:bg-slate-700/40'
                } ${collapsed ? 'justify-center' : ''}`
              }
            >
              <Icon className="w-4 h-4 shrink-0" strokeWidth={1.75} />
              {!collapsed && <span className="text-xs flex-1">{label}</span>}
