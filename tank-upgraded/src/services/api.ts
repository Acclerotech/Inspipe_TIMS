...
export const reportApi = {
  ...
  sign: (reportId: string, mfaToken: string) =>
    api.post<void>(`/reports/${reportId}/sign`, { mfaToken }),
...
export const calculationApi = {
  ...
  applyOverride: (tankId: string, overrideRate: number, reason: string) =>
    api.post<any>('/calculations/override', { tankId, overrideRate, reason }),
};
...
export const ingestionApi = {
  ...
  // FIX AT-021: backend expects [{sourceColumn, timsField, required}] not Record<string,string>
  ...
  getJobStatus: (jobId: string) =>
    api.get<{ status: string; progress?: number }>(`/ingestion/${jobId}/status`),
};
