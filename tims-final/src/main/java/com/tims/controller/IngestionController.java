...
public class IngestionController {
    ...
    @Operation(summary="Step 1: Upload file")
    public ResponseEntity<IngestionJobResponse> upload(
            @RequestPart("file") MultipartFile file,
            @RequestPart("metadata") @Valid UploadIngestionRequest req,
            @RequestParam(required = false) Integer jobId) { // Add this param
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ingestionJobService.upload(file, req, jobId)); // Pass jobId here
    }
    ...
    public ResponseEntity<IngestionStatusResponse> getJobStatus( // 1. Change return type
                                                                 @PathVariable Integer jobId
    ) {
        return ResponseEntity.ok(
                ingestionJobService.getStatus(jobId) // 2. Call getStatus() instead of getJob()
        );
    }
}
