...
public class AlertController {
    ...
    @GetMapping("/unread")
    @Operation(summary = "All unread alerts for current user — alias for GET /alerts/my",
            description = "Maps directly to the existing /my endpoint logic.")
    public ResponseEntity<Page<InspectionAlert>> getUnread(
            @PageableDefault(size = 20) Pageable pageable) {
        var email = SecurityContextHolder.getContext().getAuthentication().getName();
        var user = userRepository.findByEmail(email)
                .orElseThrow(() -> new ResourceNotFoundException("User: " + email));
        return ResponseEntity.ok(
                alertRepository.findByAssignedToIdAndReadFalseOrderByCreatedAtDesc(
                        user.getId(), pageable));
    }
