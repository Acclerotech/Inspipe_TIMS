...
public class ComplianceReportController {
    ...
    // ── NEW: PATCH /reports/{id}/status ────────────────────────────────────────
    // Status: CREATE NEW — handles the frontend workflow (REVIEWED, REJECTED, APPROVED, etc.)

    @PatchMapping("/{id}/status")
    @Operation(summary="Update report workflow status",
            description="Transitions the report through CREATED, REVIEWED, PENDING_APPROVAL, etc.")
    @PreAuthorize("hasAnyRole('ADMIN','INTEGRITY_MANAGER','INTEGRITY_ENGINEER')")
    public ResponseEntity<ComplianceReportResponse> updateStatus(
            @PathVariable Integer id,
            @RequestBody Map<String, String> payload) {

        String newStatus = payload.get("status");

        // This assumes you add an updateStatus method to your ComplianceReportService
        return ResponseEntity.ok(reportService.updateStatus(id, newStatus));
    }
}
