...
public class AssetService {
    ...
    public Map<String, Object> getAssets(int page, int size) {

        Pageable pageable = PageRequest.of(page, size);

        Page<Tank> tankPage = tankRepository.findAll(pageable);

        List<Map<String, Object>> content = tankPage.getContent()
                .stream()
                .map(tank -> {
                    Map<String, Object> row = new HashMap<>();
                    row.put("id", tank.getId());
                    row.put("tankId", tank.getTankId());
                    row.put("tankName", tank.getTankId()); // Placeholder until we join with Asset for the name
                    row.put("site", tank.getSite().getName());
                    row.put("service", tank.getProductService().getName());
                    return row;
                })
                .toList();
        ...
        response.put("totalElements", tankPage.getTotalElements());
        response.put("totalPages", tankPage.getTotalPages());
        response.put("size", tankPage.getSize());
        response.put("number", tankPage.getNumber());
