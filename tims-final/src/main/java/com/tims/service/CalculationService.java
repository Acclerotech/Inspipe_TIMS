...
public class CalculationService {
    ...
    @Transactional(readOnly = true)
    public List<CalculationResponse> getHistory(String tankId) {
        if (!tankRepository.existsByTankId(tankId)) throw new ResourceNotFoundException("Tank", tankId);
        return assessmentRepository.findFirstByTank_TankIdOrderByAssessmentDateDesc(tankId)
    ...
    public CalculationResponse recompute(Integer assessmentId) {
        ...
        var overrideOpt = overrideRepository.findActiveByTankId(ca.getTank().getTankId());
        if (overrideOpt.isPresent()) {
            var ov = overrideOpt.get();
            log.info("Applying override for tank {} in recompute", ca.getTank().getTankId());
    ...
    public CorrosionOverrideResponse applyOverride(OverrideRequest req) {
        ...
        overrideRepository.findActiveByTankId(tank.getTankId()).ifPresent(existing -> {
        ...
        var override = CorrosionOverride.builder()
                .tank(tank)
                .defaultAllowanceMm(BigDecimal.valueOf(6.0))
        ...
        auditService.record(
                ...
                AuditEvent.Action.OVERRIDE,
                null,
                saved,
                req.getReason()
        );

        return CorrosionOverrideResponse.builder()
                .id(saved.getId().longValue())
                .tankId(saved.getTank().getTankId())
                .defaultAllowanceMm(saved.getDefaultAllowanceMm())
                .overrideAllowanceMm(saved.getOverrideAllowanceMm())
                .reason(saved.getReason())
                .overriddenBy(saved.getOverriddenBy().getEmail())
    ...
    public void recomputeForTank(Tank tank) {
        ...
        var assessmentOpt = assessmentRepository.findLatestByTankId(tank.getTankId());
        ...
        BigDecimal localMaxRate = computeLocalMaxCorrosionRate(tank.getTankId());
     ...
     */
    private BigDecimal computeLocalMaxCorrosionRate(String tankId) {
        List<com.tims.entity.UtReading> readings =
                utReadingRepository.findByTank_TankIdOrderByMeasuredAtAsc(tankId);
...
// ── (2) getBreakdown — CALC-003 ──────────────────────────────
     ...
     */
    @Transactional(readOnly = true)
    public CalculationBreakdownResponse getBreakdown(String tankId) {
        ...
        log.debug("CALC-003: Found assessment={} for tank {}",
                assessmentOpt.isPresent(),
                tank.getTankId());

        if (assessmentOpt.isEmpty()) {
            throw new ResourceNotFoundException(
                    "CorrosionAssessment for tank",
                    tankId
            );
        }
        ...
        // ✅ SAFE OVERRIDE FETCH
        var overrides =
                overrideRepository.findActiveByTankId(
                        tank.getTankId()
                );
        log.debug("CALC-003: Found {} active overrides for tank {}",
                overrides.get().getOverrideAllowanceMm(), tank.getTankId());
        var overrideOpt = overrides.stream().findFirst();

        BigDecimal localMaxRate =
                computeLocalMaxCorrosionRate(tank.getTankId());

        boolean actionRequired =
                ca.getShellRemainingLifeYr() != null
                        && ca.getShellRemainingLifeYr().doubleValue() <= 0;

        String recommendation =
                buildInspectionRecommendation(ca, actionRequired);
        log.debug("CALC-003: Inspection recommendation for tank {}: {}",
                tank.getTankId(), recommendation);
        return CalculationBreakdownResponse.builder()
                ...
                .overrideAllowanceMm(
                        overrideOpt
                                .map(CorrosionOverride::getOverrideAllowanceMm)
                                .orElse(null)
                )

                .overrideReason(
                        overrideOpt
                                .map(CorrosionOverride::getReason)
                                .orElse(null)
                )
                ...
                .overriddenBy(
                        overrideOpt
                                .map(o ->
                                        o.getOverriddenBy() != null
                                                ? o.getOverriddenBy().getEmail()
                                                : null
                                )
                                .orElse(null)
                )
    ...
    private String buildInspectionRecommendation(
            CorrosionAssessment ca, boolean actionRequired) {
        if (actionRequired)
